...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for a in self.all_actions:
            node = PgNode_a(a)
            if set(node.prenodes).issubset(self.s_levels[level]):
                actions.append(node)
        self.a_levels.append(set(actions))
    ...
    def add_literal_level(self, level):
        ...
        for node in self.a_levels[level-1]:
            for pgnodes in node.effnodes:
                literals.append(pgnodes)
        self.s_levels.append(set(literals))
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a):
        ...
        if node_a1.action.effect_add == node_a2.action.effect_rem:
            return True
        elif node_a1.action.effect_rem == node_a2.action.effect_add:
            return True
        else:
            return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        node_a1_effect_add = set(node_a1.action.effect_add)
        node_a2_effect_add = set(node_a2.action.effect_add)
        node_a1_effect_rem = set(node_a1.action.effect_rem)
        node_a2_effect_rem = set(node_a2.action.effect_rem)
        node_a1_precond_pos = set(node_a1.action.precond_pos)
        node_a2_precond_pos = set(node_a2.action.precond_pos)
        node_a1_precond_neg = set(node_a1.action.precond_neg)
        node_a2_precond_neg = set(node_a2.action.precond_neg)

        if node_a1_effect_add.intersection(node_a2_precond_neg):
            return True
        elif node_a1_effect_rem.intersection(node_a2_precond_pos):
            return True
        elif node_a2_effect_rem.intersection(node_a1_precond_pos):
            return True
        elif node_a2_effect_add.intersection(node_a1_precond_neg):
            return True
        else:
            return False
